...
When the morse code has been transformed to ones and zeros the list gets scrambled with a gaussian distribution with the mean of 0 and a standard deviation of 2. They substract 0.5 from these scrambled floats and write that to a list. They note that only the flag is being transmitted over and over. We can use that to see if a certain spacing between the bits shows a similar pattern to 10 * ones and 30 * zeros.

The python script noisy.py checks for different message lengths. With each message length the median or mean can be calculated (after adding the 0.5) over all the bits in their position to approach the unscrambled bit (using message length steps of 10 because the bits follow this pattern). A check on the before mentioned patterns returned 9 possible message lengths for with a length of 960 bits being obviously the right one based on its pattern. The mean seems to result in more possible lengths but return a cleaner result. Transmission bits with a message length of 960 bits:

```

```

The mean wasn't able to fully restore the data, with 3 manual edits:
