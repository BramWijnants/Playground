...
The python script noisy.py checks for different message lengths. With each message length the median or mean can be calculated (after adding the 0.5) over all the bits in their position  to approach the unscrambled bit. Using steps of 10 because the bits follow this pattern. A check on the before mentioned patterns returned 9 possible message lengths for with a length of 960 bits being obviously the right one based on its pattern. Transmission message with a length of 960 bits:
